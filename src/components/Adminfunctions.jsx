...
const Adminfunctions = () => {
    ...
    const items = [
      {
        ...
        icon: "📁",
        color: "green",
        onClick: () => navigate('/admin/add-movie'), // Navigate to AddMovie component
      },
      {
        label: "Edit Movie",
        icon: "🔧",
        color: "blue",
        onClick: () => navigate('/admin/edit-movie'), // Navigate to EditMovie component
      },
      {
        label: "Remove Movie",
        icon: "🗑",
        color: "red",
        onClick: () => navigate('/admin/remove-movie'), // Navigate to RemoveMovie component
      },
    ];
    ...
    const GlassIcons = ({ items, className }) => {
      ...
      return (
        <div
          className={`flex gap-[5em] justify-center py-[3em] overflow-x-auto`}
        >
          {items.map((item, index) => (
            <Link to={buttonevent(index)}
              key={index}
              type="button"
